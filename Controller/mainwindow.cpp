...
void MainWindow::draw()
{
    ...
    // redraw letters
     ...
     static bool winWindowHasBeenShown = false;
     static bool loseWindowHasBeenShown = false;
     if(_game->won() && !winWindowHasBeenShown)
     {
         winWindowHasBeenShown = true;

         QMessageBox msg;
         msg.setIconPixmap(QPixmap(":/winLose/winAndLoseImages/win.jpg"));
         msg.show();
         delay();
     }
     if(!_game->won())
     {
         winWindowHasBeenShown = false;
     }

     if(_game->lost() && !loseWindowHasBeenShown)
     {
         loseWindowHasBeenShown = true;
         ...
         QMessageBox msg;
         msg.setIconPixmap(QPixmap(":/winLose/winAndLoseImages/lose.gif"));
         msg.show();
         delay();
     }
     if(!_game->lost())
     {
         loseWindowHasBeenShown = false;
     }
}
